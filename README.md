...
* **只是基础知识，没有花哨或争论**:
    ...
    - 值得注意的是，在最初的几堂课中，我们不使用集成开发环境（IDEs）。 RStudio、ESS等是很好的工具，但您不应该同时学习R和学习IDE，这会分散注意力，阻碍您尽快在R中变得高效的目标。请注意，即使是由[R-Ladies Sydney](https://threadreaderapp.com/thread/1119025557830684673.html)提供的优秀课程，也在使用RStudio，但对RStudio的评价是“**令人不知所措**”。

      因此，在最初的几堂课中，我们坚持使用R命令行，并专注于数据分析，而不是诸如IDE之类的工具，这将作为中级主题进行讲解。 （本教程的一些读者可能已经在使用RStudio或外部编辑器，对于他们来说，他们只需在RS控制台窗格中运行。。）

    - 覆盖范围主要限于基本的R。例如，自诩为“有主张”的Tidyverse并没有得到处理，部分原因是因为它具有争议性质（我是一个[Tidyverse怀疑者](http://github.com/matloff/TidyverseSkeptic)），但主要原因是它会成为您快速掌握R的障碍。虽然你可以迅速学到一些简单的、"消毒过的"东西，认为你学到了很多，但这些东西的范围相当有限，Tidy学习者通常在将R应用于实际情况时会遇到困难。我们的教程是为那些目标是*在他们自己的数据分析中高效使用R系统*的学习者而设计的。
    - **专业提示部分：** 陷阱、便捷技巧、一般建议。
...
在向量之后，R 中的下一个主要工具是**数据框**。它是一个由每个数据点一行组成的矩形表格。

假设我们有100个人的身高、体重和年龄。我们的数据框将有100行和3列。例如，第二行第三列的条目将是我们数据中第二个人的年龄。第二行作为一个整体将包含该第二个人的所有数据，即该人的身高、体重和年龄。

**请注意，该行也被视为一个向量。第三列作为一个整体将是我们数据集中所有年龄的向量。**

作为我们的第一个例子，考虑 R 内置的 **ToothGrowth** 数据集。同样，您可以通过键入以下命令在在线帮助中了解有关它的信息：

```r
> ?ToothGrowth
```

这些数据实际上是关于豚鼠的，使用橙汁或维生素 C 作为生长补充剂。让我们从命令行快速查看一下。
...
R 的 **head** 函数（默认情况下）显示给定数据框的前6行。我们可以看到有长度、补充剂和剂量三列，数据的管理者决定将它们命名为 'len'、'supp' 和 'dose'。每列都是一个 R 向量，或者在第二列的情况下，是一个称为*因子*的向量类对象，稍后会讨论）。

> 📘 专业提示
>
> 为了避免重复写出长单词，将其复制并使用一个更短的名称是很方便的。
...
> ❄️ 轮到你了
>
> 上面的例子对于 R 语言来说非常基础，所以这一次你应该自己进行一些小实验，对上面的例子进行微小的变体。你做得越多，效果就会越好！
...
> ❄️ 该轮到你了
>
> 用 **ToothGrowth** 数据构思一些小例子。例如，编写代码来查找牙齿长度小于16的情况的数量，并找出其中服用 VC 补充剂的行。
> 
> 另外，尝试一些关于另一个内置的 R 数据集 **faithful** 的例子。这个数据集涉及美国黄石国家公园中的老忠诚间歇泉。第一列给出了喷发持续时间，第二列是距上一次喷发的等待时间。
>
> 例如，编写代码来找到持续时间的最大值，并且编写代码确定该最大持续时间值的索引（第1、2、3位，等等）。
>
> 正如提到的，这些操作是 R 的关键特性，所以构思并运行尽可能多的示例；尽量做得多一些！


### 小结：在本课中我们学到了什么？
如前所述，数据框是 R 的基本工具。它由列向量组成（长度相等），这一事实通常非常有用。

与向量的单个数字索引不同，数据框中的每个元素都有两个索引，一个是行号，一个是列号。可以指定行和列的集合来提取子框架。

可以使用 R 的 **nrow** 函数查询数据框中的行数；**ncol** 函数用于查询列数。
...
> ❄️ 轮到你了
>
> 在 **mtcars** 数据中，拟合一个线性模型，将 MPG 对重量和马力进行回归分析；对于固定的马力，额外增加100磅的重量的估计效果是什么？
